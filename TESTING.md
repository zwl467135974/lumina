...
> **v1.2 更新**：项目已建立完整自动化测试体系（103 测试）。
> 运行 `mvn test` 即可执行全部测试，无需手动建表/初始化（Flyway 自动迁移 V1-V6）。
> 测试分布：common 28 + agent-core 23 + framework 4 + base 38 + agent 10。
>
> 下方为早期手动验证步骤（已被自动化测试覆盖，保留作参考）。
