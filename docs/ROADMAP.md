...
**版本**: v1.2.0
**最后更新**: 2026-07-02
**当前阶段**: v1.2 功能全部完成（RAG + 可观测性 + 前端增强 + 部署文档）

---

## v1.1 已完成清单（2026-07-01）

| ROADMAP 项 | 内容 | 提交 |
|-----------|------|------|
| 2.1 响应式上下文 | ContextPropagation + ThreadLocalAccessor | 早期 |
| 2.2 敏感配置治理 | 环境变量占位 + .env.example | 早期 |
| 2.3 统一错误码 | ErrorCode 枚举 + 全量迁移 | 早期 |
| 4.1 流式输出 | SSE 端到端（后端 Flux + 前端打字机） | 早期 |
| 4.2 会话与记忆 | 多轮对话 + 历史回放 + token 用量 | c4a9be4 |
| 4.4 多模型适配 | ChatModelFactory（dashscope/openai/anthropic/ollama） | 早期 |
| 4.5 工具调用可观测 | 记录/统计/熔断器 | 18c3eee |
| 5.1 日志规范（MDC） | traceId/tenantId/userId 结构化日志 | 早期 |
| 5.3 审计日志 | @Audit AOP + 事件驱动持久化 | bd97728 |
| 6.2 Flyway | V1-V4 版本化迁移 | 早期 |
| 6.3 限流熔断 | Gateway RateLimitGlobalFilter | a42b407 |
| 7 前端体验 | 工具监控页 + 暗色主题 | a42b407 |
| 3.1 测试体系 | 单元 89 + 集成 6 = 95 测试 | 多提交 |
| 6.1 API 版本策略 | @DeprecatedApi + Deprecation/Sunset 响应头 | 451af5a |
| 5.2 Micrometer 指标 | Prometheus + Grafana + Timer 埋点 | b6f331e |

**剩余**：5.1 OpenTelemetry 全链路追踪 / 4.3 RAG 知识库 / 运维部署（K8s/灰度）
...
| 优先级 | 方向 | 预计工作量 | 价值 | 状态 |
|--------|------|-----------|------|------|
| **P0** | 响应式上下文传递 | 1-2 天 | 修复多租户异步隔离隐患 | ✅ 完成 |
| **P0** | 敏感配置治理 | 0.5 天 | 安全基线 | ✅ 完成 |
| **P1** | 单元/集成测试 | 5-7 天 | 质量保障 | ✅ 95 测试 |
| **P1** | 流式输出 + 会话管理 | 4-5 天 | 核心体验 | ✅ 完成 |
| **P1** | 多模型适配 | 2-3 天 | 扩展性 | ✅ 完成 |
| **P2** | 统一错误码 | 1 天 | 前后端协作 | ✅ 完成 |
| **P2** | 日志/追踪/指标 | 3-4 天 | 可观测性 | ✅ MDC + Micrometer + 审计 + OTel 全链路 |
| **P2** | Flyway + 限流熔断 | 2-3 天 | 工程化 | ✅ 完成 |
| **P2** | 前端对话 UI | 3-4 天 | 体验 | ✅ 完成 |
| **P2** | API 版本策略 | 0.5 天 | 工程规范 | ✅ 完成 |
| **P2** | 前端增强 | 2-3 天 | 体验 | ✅ 动态菜单 + 调试面板 + i18n |
| **P3** | RAG / 知识库 | 5-7 天 | 能力扩展 | ✅ 全 5 阶段完成 |
| **P3** | K8s / CI/CD | 3-4 天 | 运维 | ✅ 参考模板 + 部署文档 |
...
- **M2（质量基线）**: ✅ 测试 95 例（单元 89 + 集成 6），集成测试发现并修复 2 个租户隔离 bug
- **M3（体验跃升）**: ✅ 流式对话 + 会话管理 + 多模型上线
- **M4（生产可观测）**: ✅ MDC traceId + 审计 + Micrometer 指标 + OpenTelemetry 全链路追踪（Jaeger）
- **M5（能力扩展）**: ✅ RAG 知识库全 5 阶段 + CI/CD/K8s 参考模板 + 部署文档 + 启动脚本
