# Lumina 文档索引

> 中文文档为标准版本，英文文档为对应翻译。两侧结构一致。

## 中文文档（标准）

### 入门
- [快速开始](zh/快速开始.md) — 5 分钟跑起来
- [Lumina 框架说明](zh/lumina框架说明.md) — 框架全景

### 架构设计
- [项目结构设计](zh/architecture/项目结构设计.md)
- [Lumina 模块设计](zh/architecture/Lumina模块设计.md)
- [Lumina 技术选型方案](zh/architecture/Lumina技术选型方案.md)
- [Agent 执行引擎设计](zh/architecture/Agent执行引擎设计.md)
- [前端架构设计](zh/architecture/前端架构设计.md)
- [架构模式分析与建议](zh/architecture/架构模式分析与建议.md)

### 开发指南
- [开发规范与编码标准](zh/guides/Lumina开发规范与编码标准.md)
- [Agent 开发指南](zh/guides/Agent开发指南.md)
- [工作流设计指南](zh/guides/工作流设计指南.md)
- [业务模块开发指南](zh/guides/业务模块开发指南.md)
- [前端开发指南](zh/guides/前端开发指南.md)
- [工具开发指南](zh/guides/工具开发指南.md)
- [配置管理规范](zh/guides/配置管理规范.md)
- [数据库配置指南](zh/guides/数据库配置指南.md)

### 部署运维
- [部署指南](zh/deployment/部署指南.md) — Docker Compose + K8s
- [standalone 部署](zh/deployment/standalone部署.md) — 单体模式 5 分钟启动（仅 MySQL+Redis）
- [配置说明](zh/deployment/配置说明.md) — JWT、白名单、租户隔离等
- [Nacos 动态路由验证](zh/deployment/Nacos动态路由验证.md)

### 设计文档
- [RAG 架构设计](zh/design/RAG架构设计.md)
- [文件存储设计](zh/design/文件存储设计.md)
- [Prompt 运行时规则](zh/design/Prompt运行时规则.md)
- [Agent 评估框架设计](zh/design/E1-Agent评估框架设计方案.md)
- [推理链可观测性实施方案](zh/design/推理链可观测性实施方案.md) — Trace 树埋点 + 步骤级成本归因

### 路线图
- [v2.0 路线图](zh/roadmap/v2.0路线图.md)
- [v3.0 路线图](zh/roadmap/v3.0路线图.md)
- [前端功能完善路线图](zh/roadmap/前端功能完善路线图.md)
- [生产就绪加固计划](zh/roadmap/生产就绪加固计划.md) — v3.6 企业级加固
- [远期规划](zh/roadmap/远期规划.md) — ~~multi-agent handoff~~（已实现 v3.8 Supervisor）/ 可观测性 dashboard
- [技术演进路线图](zh/roadmap/技术演进路线图.md) — 各维度成熟度评估 + AI 深入方向（三梯队）

### 战略与方案
- [市场定位分析](zh/strategy/市场定位分析.md) — vs Dify/LangGraph/Spring AI Alibaba
- [智能运维助手方案](zh/strategy/智能运维助手方案.md) — 覆盖全部 16 能力的最佳实践 demo

### 最佳实践
- [智能运维助手 Demo](../examples/ops-platform/README.md) — 端到端业务场景 + Python 脚本 + 前端操作指南

### RFC
- [Fastjson2 与 Jackson 对比分析](zh/rfc/Fastjson2与Jackson对比分析.md)

---

## English Docs (Mirror)

### Getting Started
- [Quick Start](en/QUICK_START.md)

### Architecture
- [Architecture Overview](en/ARCHITECTURE.md)

### Development
- [Agent Development Guide](en/AGENT_DEVELOPMENT.md)
- [Workflow Design Guide](en/WORKFLOW_DESIGN.md)

---

## Archive

历史文档归档于 `archive/` 目录，包含 v1.x 路线图、进度报告、任务清单等。
