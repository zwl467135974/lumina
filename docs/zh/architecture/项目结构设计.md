...
└── src/main/java/io/lumina/agent/
    ...
    │   └── MemoryManager.java
    └── workflow/                     # 工作流引擎 (v3.1+)
        ├── FlowableBpmnConverter.java     # YAML → BPMN 模型转换
        ├── AgentServiceTaskDelegate.java  # Agent 节点 Delegate
        ├── TransformServiceTaskDelegate.java  # Transform 节点 Delegate
        └── LoopServiceTaskDelegate.java   # Loop 节点 Delegate
└── src/main/resources/
    └── agent-config/
        └── default.yaml              # 默认配置
```
