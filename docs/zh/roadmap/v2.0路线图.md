...
| 工程治理 | Flyway V1-V14 + 10 个 Skill 规范 + 30+ 篇文档 + CI/CD 双流水线 |
| 部署体系 | docker-compose 13 服务 + Dockerfile 4 个 + K8s 清单 + Helm Chart |
| 测试 | 后端 355 + 前端 80 = 435 个测试用例 |
...
## 当前完成状态（截至 2026-07-07）

### 已完成

- **P0**：测试补全与技术债清理，CI 后端/前端门禁已可通过。
- **P1**：工作流编排引擎、模板 API、前端工作流管理入口（含可视化设计器）、并行执行器修复。
- **P2-1**：多模态流式接口与前端流式多模态执行。
- **P2-2**：RAG 来源可视化（`RAG_SOURCES` 流式事件 + 前端引用来源折叠面板）。
- **P2-3**：Agent 调试面板基础能力。
...
- **P3-1**：异步任务队列（线程池 + SSE 进度 + 任务列表页 + 取消）。
- **P3-2**：成本管理（价格表 + 计费 + 汇总仪表盘 + 趋势图表 + 预算管控 + 告警）。
- **P3-3**：安全防护（Prompt 注入检测 + PII 脱敏 + 频率限制 + 内容审核）。
- **P4**：Helm Chart 全量模板（lint + dry-run 验证）+ Nacos 动态路由实际验证通过。
- **P5**：英文 README + 核心文档 + Apache 2.0 License + CHANGELOG.md。
- **E1**：Agent 评估框架全栈实现（见下方 E1 章节）。
- **联调修复**：Redis 密码 / Gateway 过滤器命名 / MyBatis 租户拦截器 / 前端 API 路径 / 登录链路修复。

### 测试基线

- 后端：355 单元测试（全 8 模块 `mvn verify` 通过）
- 前端：80 测试（66 工具/Store + 14 组件），`pnpm test` + `pnpm build` 通过

### 无剩余项

所有 P0-P5 + E1 均已交付。后续仅集成测试补全为待办。
...
| 能力 | 说明 |
|------|------|
| 数据集管理 | YAML 格式 + 文件上传导入（`POST /datasets/import`） |
| 精确匹配评分 | `ExactMatchScorer`（trim 归一化） |
| 关键词包含评分 | `ContainsScorer`（命中比例） |
| 语义相似度评分 | `SemanticSimilarityScorer`（Embedding 余弦相似度，不可用时回退 Jaccard） |
| LLM Judge 评分 | `LlmJudgeScorer`（ChatModelFactory + 1-5 分制 + 归一化 0-1） |
| 评估报告 | 分类统计 + ECharts 柱状图 + 历史趋势折线图 |
| 异步评估 | 大数据集异步执行 + 2s 轮询（V14 status 字段） |
| A/B 对比 | 两次评估按用例对齐，显示分数变化和趋势 |
| CSV 导出 | 全部用例结果（含 BOM 支持 Excel） |
| Flyway | V13（dataset + run 两表）+ V14（status 字段） |
| 测试 | 评分器 8 tests + Service 2 tests |
| 设计文档 | `docs/zh/design/E1-Agent评估框架设计方案.md` |
...
| 能力 | 说明 |
|------|------|
| `EmbeddingRouter` | `implements EmbeddingModel`，透明拦截 ingest + retrieve 两条路径 |
| 语言检测 | CJK 字符占比 ≥ 30% → 中文模型，否则英文模型 |
| 多模型配置 | `lumina.rag.router.models` Map 配置多个命名 Embedding 后端 |
| 文档元数据 | Flyway V15 增加 `language` + `embedding_model` 列 |
| 零侵入 | `RagKnowledgeFactory` bean 替换为 Router，下游 `SimpleKnowledge` 无感知 |
| 测试 | 5 tests（中文路由/英文路由/回退/维度/语言检测） |
...
| 能力 | 说明 |
|------|------|
| 知识库实体 | `lumina_knowledge_base` 表（Flyway V16），含三级可见性 PRIVATE/TEAM/PUBLIC |
| Agent 挂载 | `lumina_agent_knowledge_base` 多对多挂载表，Agent 可挂载多个 KB |
| 文档归属 | `KnowledgeDocumentDO` 新增 `kb_id` 字段 |
| 可见性规则 | Agent 可访问 = 挂载的 KB + 同租户 PUBLIC KB |
| 管理 API | 知识库 CRUD + Agent 挂载/卸载 + 查询 Agent 可用 KB |
| 前端 | 知识库联邦页面（列表 + 创建 + 挂载管理） |
...
- [x] E1：Agent 评估框架（4 种评分器 + 分类柱状图 + 趋势折线图 + 异步评估 + A/B 对比 + CSV 导出）
- [x] 后端全量测试通过（355 单元测试 + JaCoCo 门禁）
- [x] 前端全量测试通过（80 测试）
- [x] CHANGELOG.md 更新
- [x] README.md 更新版本和特性
...
| 测试覆盖 | ~10%（135 用例） | ≥ 40%（355 后端 + 80 前端 = 435 用例） |
