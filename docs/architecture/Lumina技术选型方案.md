...
| 技术 | 版本 | 选型理由 | 参考来源 |
|------|------|---------|---------|
| **Java** | **21 (LTS)** | 最新 LTS 版本，性能提升显著，支持虚拟线程 | lumina-ref 使用 |
...
| 技术 | 版本 | 选型理由 | 参考来源 |
|------|------|---------|---------|
| **Spring Boot** | **3.3.5** | 稳定版本，与 Java 21 完美兼容 | lumina-ref |
| **Spring Cloud** | **2023.0.3** | 与 Spring Boot 3.3.5 匹配 | lumina-ref |
| **Spring Cloud Alibaba** | **2023.0.1.2** | 提供 Nacos、Sentinel 等微服务组件 | lumina-ref |
...
| 技术 | 版本 | 选型理由 | 参考来源 |
|------|------|---------|---------|
| **MyBatis** | **3.0.3** | 成熟稳定，灵活度高，适合复杂查询 | lumina-ref |
| **MyBatis-Plus** | **3.5.7+** | 提供 CRUD 增强，减少样板代码 | 建议新增 |
| **PageHelper** | **2.1.0** | 分页插件，简单易用 | lumina-ref |
| **Druid** | **1.2.23** | 阿里数据库连接池，监控功能强大 | lumina-ref |
| **Dynamic DataSource** | **4.3.1** | 多数据源支持，适合多租户场景 | lumina-ref |
...
| **Redisson** | **3.24.3** | Redis Java 客户端，提供分布式锁等高级特性 | lumina-ref |
| **Kafka** | **3.6+** | 高吞吐量消息队列，适合事件驱动架构 | lumina-ref |
...
| 技术 | 版本 | 选型理由 | 参考来源 |
|------|------|---------|---------|
| **Nacos** | **3.1.1+** | 服务注册、配置管理、服务发现 | agentscope-java / lumina-ref |
...
| 技术 | 版本 | 选型理由 | 参考来源 |
|------|------|---------|---------|
| **SpringDoc (Swagger)** | **2.6.0** | OpenAPI 3.0 文档生成 | lumina-ref |
...
| **Fastjson2** | **2.0.53** | 高性能 JSON 处理 (可选) | lumina-ref |
...
**说明**:
- ⚠️ lumina-ref 使用 jjwt 0.9.1 (较旧)，建议升级到 0.12.5
...
| **Logback + Logstash** | 最新 | 日志收集 | lumina-ref |
...
| **MinIO** | **8.2.2+** | 对象存储 (可选) | lumina-ref |
...
| 技术 | 版本 | 选型理由 | 参考来源 |
|------|------|---------|---------|
| **Vue 3** | 3.x | 现代化前端框架 | lumina-ref (Vue 2) |
| **Element Plus** | 最新 | UI 组件库 | 建议升级 |
| **TypeScript** | 5.x | 类型安全 | 建议新增 |

**说明**:
- ⚠️ lumina-ref 使用 Vue 2，建议升级到 Vue 3
- ✅ 前后端分离架构
...
2. **JSON 处理库**
   - AgentScope 使用 Jackson，lumina-ref 使用 Fastjson2
...
- ✅ **lumina-ref**: 作为参考，避免落后技术，参考其项目结构减少 bug
