...
- **4.2 Flyway 持久化变更说明书制度**：`lumina_flyway` skill 新增强制章节——每个迁移头部三行说明书（变更/回滚/影响），不可逆变更显式声明 + 备份语句、种子回滚按条件、多表逐表列明、commit 描述联动、存量行默认值语义（ADR-002 先例）
- **4.3 确定性防线清单（B3）+ 中断语义边界（B4）**：新增《确定性防线清单》——13 项代码强制防线 vs 5 类提示词约定的完整分类（"提示注入能否绕过 X"的判定标准），含钩子 fail-open 边界注记；B4 收口 A6 真取消的保证/尽力而为两清单（记忆闭合/中断标记/上下文清理有保证；工具线程协作式放弃/续段中途取消/转向消息保留等边界语义明示）与上游差异说明（无进程边界可杀，真取消落在流切断+记忆闭合+清理三层）
- 新增 9 个用例（解析/打分夹取/摘录纪律/白名单双闸门/幂等跳过/租户隔离/上下文清理）；business-agent 362 全绿
