...
### 🤖 纯 AI 开发的工程实践范本（不止学框架，学"怎么用 AI 做工程"）

本项目的绝大部分代码由 **AI 编写、人类定向**完成——而支撑这种开发方式的**方法论资产全部随仓库开源**，是比项目内容本身更有普适价值的实践教学：

- **[AGENTS.md 纲领](AGENTS.md)** - AI 开发的"项目宪法"（155 行，四份镜像文件跨工具同步）：模块结构、编码规范、Git 推送/提交规则，以及 20+ 项**后端编码前置检查清单**（分层完整性/事务/错误码/权限/审计/编译验证）——AI 每次写代码前先过清单，把"资深工程师的 review 经验"固化成 AI 可执行的纪律
- **13 个定制技能包**（[`.agents/skills/`](.agents/skills/)，2500+ 行规范沉淀）- 把项目规范拆成 AI 可按需加载的技能：代码风格、分层架构、MyBatis-Plus、API 设计、领域建模、JSON 序列化、Git 提交、测试、可观测性、会话记忆、前端设计、Redis、Flyway——写什么代码加载什么技能，精准注入而非塞满上下文
...
- 📄 **输入侧上下文外存** ✅ — 超阈值历史消息（粘贴代码块/长文档/大结果）全文存档留预览 + artifactId、`util.getArtifact` 按需水合，长会话不再逐轮为同一大消息付费；存档失败保留原文（宁可贵不失真），A5 spill 的输入侧镜像（双上游验证）
